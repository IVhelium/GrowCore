...
The responsive React SPA provides a service top bar, desktop/mobile header, catalog/search navigation, account popover, notification and friend-request counters, mobile drawer and bottom navigation, category sidebar, footer, empty/loading/error states, global toast notices, and a back-to-top control after scroll.
...
**Product details.** The page combines a selectable image carousel with an adaptive one-line thumbnail carousel, title, rating, base/current price, discount, stock, and quantity controls. Quantity cannot exceed available inventory. The summary beside the price uses only the seller's Overview/manufacturer description and clamps long text to four lines. Purchase and heart buttons add the selected amount or toggle saving. Missing or broken non-avatar images render a file placeholder icon. Lower tabs expose the full structured description, with Characteristics displayed as vertical name/value rows, plus technical attributes, delivery/warranty notes, and review threads. Signed-in users can add their one rated review and reply to root reviews. A related-product section provides continued catalog discovery.
...
**Create/edit product.** Forms collect title, price, discount and expiry, stock, category, structured description sections, and filterable attributes. Creating a listing performs three explicit stages: create draft, upload required images, submit for moderation. Sellers can choose images in multiple passes; selected files remain as local draft previews until the form is saved or submitted. Editing loads current values and images, then supports draft saving or save-and-submit only when those are distinct actions. Submission validates required description sections, Brand, Warranty, meaningful characteristic values, discount limits, and non-past discount expiry. Brand and Warranty remain permanently present in the attribute editor and cannot be removed; optional attributes remain removable. Existing images are displayed and individually removable.
...
Product pages provide image carousel selection with an adaptive one-line thumbnail track, file-icon fallbacks for missing non-avatar images, base/current price, discount badge, stock and quantity selection, ratings, seller/category, clamped Overview summary, structured description, attributes, cart/favorite controls, delivery/warranty panels, review threads, and related products. Discounts range from 0–100% in backend calculation, while seller forms restrict manual entry to 0–99%, may expire, and produce a two-decimal computed price.
...
Each seller owns one store with editable name and description. Listing lifecycle is `draft → pending → approved`, plus `rejected`, `blocked`, and `deleted` states. Sellers create structured drafts, add multiple JPEG/PNG/WebP images up to 8 MiB each with local previews, edit draft/rejected listings, submit for moderation, toggle approved availability, and delete unpublished listings with a reason. Zero-stock listings cannot be enabled. Rejected edits return to draft.
